        ...
        <div className="mt-3 grid gap-1">
            {!poll.voted && poll.answers.map((answer) => (
                <button
                    ...
                    className="h-8 border border-[#1d9bf0] rounded-full text-[#1d9bf0] font-bold hover:bg-[#1d9bf06a] transition-colors"
                    key={answer.id}
                >
                    {answer.text}
                </button>
            ...
            {poll.voted && poll.answers.map((answer) => (
                <div className="h-8 flex items-center justify-between relative overflow-hidden rounded-md px-2 z-[1]">
                    <div
                        ...
                        className="absolute top-0 left-0 h-full bg-[#71767b] opacity-50 z-[-1]"
                    />
